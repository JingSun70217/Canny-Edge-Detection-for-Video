...
-	Preprocessing:  Equirectangular to gnomonic projection
...
-	Preprocessing:
 ...
 >Convert the annotation results format.  
...
(I will add a Flowchart for pipeline in here later)
...
## Input data
The panorama images and distance images can be downloaded manually from our [SCENE WebShare Cloud](https://arena2036.3d-enterprise.com/) website or downloaded automatically by running “ws-pano-image-downloader” scripts. 
...
Please run `/frustum_proposal_generation/request_TransformationGlobal/get_TransformationGlobal.py`to get the transformation matrix at first.
The run `/frustum_proposal_generation/generate3DBbox.py`to get the annotation results.

**Convert the annotation results format**

- Data augmentation
- Convert annotation results to Kitti format which can be used by the Frustum Pointnets directly.
...
Please run `/3D_frsutum_pointNets/train/train.py` at first. The run `/3D_frsutum_pointNets/train/test.py`
